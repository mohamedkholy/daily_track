...
class AddRoutineRepo {
  ...
  Future<bool> addRoutine(Routine routine) async {
    try {
      ...
      await Future.wait(routine.tasks!.map((task) async => await addTask(
          task, routine.title)));
      return true;
    } catch (e) {
      return false;
  ...
  Future<void> addTask(Task task, String routineTitle) async {
    ...
    await Future.wait(task.subTasks.map((subTask) async => await addSubtask(
        subTask, task.title, routineTitle)));
  }

   addSubtask(Subtask subtask,
      String taskTitle,
      String routineTitle,) async {
    await db
        .into(db.subtasks)
        .insert(
